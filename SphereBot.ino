...
/* inverted. This is to map better to downloadable things from thingiverse.
 *
 * basically X is now the rotating-egg axis
 * and Y is the pen wiggle waggle axis
 *
 * not inverted no mo' 'cause egg displacement correction code is pinned to Y axis.
 */
...
/* Pins, final. This is pinned for what I ended up soldering. */

#define XAXIS_DIR_PIN 8
#define XAXIS_STEP_PIN 7
#define XAXIS_ENABLE_PIN 6
#define XAXIS_ENDSTOP_PIN -1

#define YAXIS_DIR_PIN 12
#define YAXIS_STEP_PIN 10
#define YAXIS_ENABLE_PIN 9
...
#define DEFAULT_PEN_UP_POSITION 12
...
/* In case you want the Y to be the wiggle-waggle axis.
*/
#define YAXIS_MIN_STEPCOUNT -467
#define YAXIS_MAX_STEPCOUNT 467

#define DEFAULT_ZOOM_FACTOR 1.0 // I don't understand the maths. Basically, inkscape max values for wiggle-waggle axis is ~140
                                 // and imperically my max is (-30 to 30). So I set scaling to 0.22
																 // the goal here is to support 3200x800 px images to "just print correctly, damnit".
																 // didnt work though. 0.10 seems somewhat closer to reasonable
																 // but this scales the around-the-egg incorrectly...
...
/* This is if you want Y to be the wiggle waggle axis */
    ...
    {   
      ...
      case 300: // Servo Position
        if(getValue('S', command, &value))
        {
          ...
          oldvalue = (int)servo.read();
          value = (int)value;
          if( oldvalue < value )
          {
              servo.write((int)(.5*value));
              oldvalue = (int)servo.read();
              for(int pos = oldvalue; pos <= value; pos += 1)
              {
                  servo.write(pos);
                  SoftwareServo::refresh();
                  delay(delayValue);
